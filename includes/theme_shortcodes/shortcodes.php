<?php
// container_24
function shortcode_container_24($atts, $content) {
extract(shortcode_atts(array(
				'custom_class' => ''
		), $atts));
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="container_24 clearfix '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('container_24', 'shortcode_container_24');
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_1 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_2 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_3 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_4 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_5 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_6 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_7 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_8 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_9 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_10 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_11 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_12 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_13 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_14 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_15 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
...
extract(shortcode_atts(array(
	...
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_16 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('grid_16', 'shortcode_grid_16');

// grid_17
function shortcode_grid_17($atts, $content) {
extract(shortcode_atts(array(
				'custom_class' => ''
		), $atts));
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_17 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('grid_17', 'shortcode_grid_17');

// grid_18
function shortcode_grid_18($atts, $content) {
extract(shortcode_atts(array(
				'custom_class' => ''
		), $atts));
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_18 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('grid_18', 'shortcode_grid_18');

// grid_19
function shortcode_grid_19($atts, $content) {
extract(shortcode_atts(array(
				'custom_class' => ''
		), $atts));
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_19 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('grid_19', 'shortcode_grid_19');

// grid_20
function shortcode_grid_20($atts, $content) {
extract(shortcode_atts(array(
				'custom_class' => ''
		), $atts));
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_20 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('grid_20', 'shortcode_grid_20');

// grid_21
function shortcode_grid_21($atts, $content) {
extract(shortcode_atts(array(
				'custom_class' => ''
		), $atts));
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_21 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('grid_21', 'shortcode_grid_21');

// grid_22
function shortcode_grid_22($atts, $content) {
extract(shortcode_atts(array(
				'custom_class' => ''
		), $atts));
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_22 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('grid_22', 'shortcode_grid_22');

// grid_23
function shortcode_grid_23($atts, $content) {
extract(shortcode_atts(array(
				'custom_class' => ''
		), $atts));
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_23 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('grid_23', 'shortcode_grid_23');

// grid_24
function shortcode_grid_24($atts, $content) {
extract(shortcode_atts(array(
				'custom_class' => ''
		), $atts));
	//remove wrong nested p tags
	$content = remove_invalid_tags($content, array('p'));
	return '<div class="grid_24 '.$custom_class.'">'.do_shortcode($content).'</div>';
}
add_shortcode('grid_24', 'shortcode_grid_24');
...
function shortcode_recent_posts($atts, $content = null) {
		...
		foreach($latest as $post) {
				...
				if($more_text_single!=""){
					$output .= '<a href="'.get_permalink($post->ID).'" class="button1" title="'.get_the_title($post->ID).'">';
...
function shortcode_audio($atts, $content = null) {
    ...
    if($mp3 !== false){
        ...
        _e("The URL you entered is not a .mp3 file.", "theme1336");
...
function shortcode_video($atts, $content = null) {
    ...
    if($vimeo !== false){
        ...
        _e("You entered a video URL that isn't compatible with this shortcode.", "theme1336");
