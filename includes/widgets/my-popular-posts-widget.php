...
class MY_PopularPostsWidget extends WP_Widget {
                  ...
                  $pop_posts = $count;
                    ...
                    if($posts){
                      foreach($posts as $post){
                        ...
                        $thumb = $thumb[0];
												$excerpt = stripslashes($post->post_content);
                			?>
                        <li>
                          <?php if ($thumb) { ?>
                            <figure class="post-thumb">
                              <a href="<?php echo $guid; ?>"><img src="<?php echo $thumb; ?>" /></a>
                            </figure>
                          <?php } else { ?>
                          	<figure class="post-thumb empty-thumb"></figure>
                          <?php } ?>
                          <h4><a href="<?php echo $guid; ?>" title="<?php echo $post_title; ?>"><?php echo $post_title; ?></a></h4>
                          <div class="excerpt"><?php echo my_string_limit_char($excerpt,50);?>... <a href="<?php echo $guid; ?>" class="link1"><?php _e('more','theme1336');?></a></div>
                        </li>
                    <?php
                        }
                    }
    ...
    ?>
      ...
      <p><label for="<?php echo $this->get_field_id('linktext'); ?>"><?php _e('Link Text:', 'theme1336'); ?> <input class="widefat" id="<?php echo $this->get_field_id('linktext'); ?>" name="<?php echo $this->get_field_name('linktext'); ?>" type="text" value="<?php echo $linktext; ?>" /></label></p>
			 
			 <p><label for="<?php echo $this->get_field_id('linkurl'); ?>"><?php _e('Link Url:', 'theme1336'); ?> <input class="widefat" id="<?php echo $this->get_field_id('linkurl'); ?>" name="<?php echo $this->get_field_name('linkurl'); ?>" type="text" value="<?php echo $linkurl; ?>" /></label></p>
